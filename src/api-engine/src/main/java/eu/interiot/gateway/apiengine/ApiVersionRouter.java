...
public class ApiVersionRouter{
	...
	@ApiOperation(value = "Get Api version information", response = String.class)
    public Response version() {
		try {
			String version = GatewayInfo.localInstance().getSpecificationVersion();
			int idx = version.indexOf("-");
			if(idx > 0) version = version.substring(0, idx);
			return ApiResponse.jsonResponse(200, version).build();
		} catch (Exception e) {
